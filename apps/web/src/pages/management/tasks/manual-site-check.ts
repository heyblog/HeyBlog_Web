...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  const response = await fetch(`${getApiBaseUrl()}/api/management/tasks/manual/site-check`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    return new Response('Failed to enqueue manual site check', { status: response.status });
  }
