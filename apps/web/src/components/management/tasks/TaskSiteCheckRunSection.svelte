<script lang="ts">
  ...
  import {
    buildProbeEntries,
    buildRunResultClass,
    buildSiteCheckStatusOptions,
    buildSiteCheckSummaryEntries,
  ...
  let statusOptions = $derived(buildSiteCheckStatusOptions(runs));
  let filteredRuns = $derived(
    runs.filter((row) => {
      const matchesStatus = !status || row.availability_result === status;
      const matchesSite =
...
<section class="rounded-md border border-(--color-line) bg-(--color-bg-raised) p-5">
  <div class="flex flex-wrap items-end justify-between gap-3">
    <div>
      <h2 class="text-base font-semibold text-(--color-fg)">站点检测运行摘要</h2>
      <p class="mt-1 text-sm text-(--color-fg-3)">按站点查看本次 job 的国内和国外探测摘要。</p>
    </div>
          ...
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class={`text-sm ${buildRunResultClass(row.availability_result)}`}
                  >{row.availability_result}</span
                >
            ...
            <p class={`text-sm ${buildRunResultClass(row.verify_result)}`}>{row.verify_result}</p>
          </div>

          <div class="mt-4 space-y-4">
            <TaskDetailFieldGrid
              entries={buildSiteCheckSummaryEntries(row)}
              columnsClass="md:grid-cols-2 xl:grid-cols-3"
            />

            <div class="grid gap-4 xl:grid-cols-2">
              {#each buildProbeEntries(row) as probe (probe.id)}
                <section class="rounded-md bg-(--color-bg-raised) px-4 py-4">
                  <div class="flex flex-wrap items-center justify-between gap-2">
                    <h4 class="text-sm font-medium text-(--color-fg)">{probe.label}</h4>
                    <span class={`text-sm ${probe.statusClass}`}>{probe.status}</span>
                  </div>
                  <div class="mt-3">
                    <TaskDetailFieldGrid
                      entries={probe.fields}
